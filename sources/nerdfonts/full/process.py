...
def scaleGroup(fileName, glyph, SIDEBEARING):
	...
	if "codicon" in fileName:
		...
		if glyph.unicode in [0xf0d7, 0xf0d8, 0xf0d9, 0xf0da, 0xf0dc, 0xf0dd, 0xf0de]: #carets all directions
			xAdjustment = (FONTWIDTH-(2*SIDEBEARING))/(scaledFont["caret_down"].bounds[2] - scaledFont["caret_down"].bounds[0])
			modified = True
		...
		if glyph.unicode in [0xF182,0xF183]: #male and female
			...
			modified = True
		if glyph.unicode in range(0xF221,0xF22D): #gender or so
		...
		if glyph.unicode in range(0xF4EF,0xF4F3): #move to
			xAdjustment = (FONTWIDTH-(2*SIDEBEARING))/(scaledFont["move-to-top"].bounds[2] - scaledFont["move-to-top"].bounds[0])
			modified = True
		if glyph.unicode in [0xF461,0xF47A,0xF493,0xF533]: #bookmarks
			xAdjustment = (FONTWIDTH-(2*SIDEBEARING))/(scaledFont["bookmark-slash-fill"].bounds[2] - scaledFont["bookmark-slash-fill"].bounds[0])
			modified = True
		...
		if glyph.unicode in [0xE300, 0xE301, 0xE302, 0xE303, 0xE304, 0xE305, 0xE306, 0xE307, 0xE308, 0xE309, 0xE30A, 0xE30B, 0xE30C, 0xE30D, 0xE30E, 0xE30F, 0xE310, 0xE311, 0xE312, 0xE313, 0xE314, 0xE315, 0xE316, 0xE317, 0xE318, 0xE319, 0xE31A, 0xE31B, 0xE31C, 0xE31D, 0xE31E, 0xE31F, 0xE320, 0xE321, 0xE322, 0xE323, 0xE324, 0xE325, 0xE326, 0xE327, 0xE328, 0xE329, 0xE32A, 0xE32B, 0xE32C, 0xE32D, 0xE32E, 0xE32F, 0xE330, 0xE331, 0xE332, 0xE333, 0xE334, 0xE335, 0xE336, 0xE337, 0xE338, 0xE33A, 0xE33B, 0xE33C, 0xE33D, 0xE342, 0xE343, 0xE346, 0xE34B, 0xE34C, 0xE34D, 0xE35C, 0xE35D, 0xE35E, 0xE35F, 0xE360, 0xE361, 0xE362, 0xE363, 0xE364, 0xE365, 0xE366, 0xE367, 0xE36A, 0xE36B, 0xE36C, 0xE36D, 0xE36E, 0xE36F, 0xE370, 0xE371, 0xE372, 0xE373, 0xE374, 0xE375, 0xE376, 0xE377, 0xE378, 0xE379, 0xE37A, 0xE37B, 0xE37C, 0xE37D, 0xE37E, 0xE3AA, 0xE3AB, 0xE3AC, 0xE3AD, 0xE3AE, 0xE3BC, 0xE3BD, 0xE3BE, 0xE3BF, 0xE3C0, 0xE3C1, 0xE3C2, 0xE3C3, 0xE345,0xE34A,0xE351]: # Various
	...
	if "Powerline" in fileName and glyph.unicode in range(0xE0B0,0xE0D5): #Powerline glyphs should be full width / full height 
		yAdjustment = 2706/height
		#xAdjustment = FONTWIDTH/width
		xAdjustment = yAdjustment
		modified = True

	if modified == False:
		if glyph.unicode == 9776:
			SIDEBEARING = FONTWIDTH*.1 # uni2630 requires additional space around it
...
#for file in ["original/codicon.ufo"]:
for file in INPUT.glob("*.ufo"):
	...
	for glyph in font:
		if glyph.bounds is not None:
			...
			if "FontAwesome" in str(file):
				heightAdjustment = 296-69
			elif "weather" in str(file):
				heightAdjustment = 297-169
